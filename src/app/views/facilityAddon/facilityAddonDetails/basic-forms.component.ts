...
// Timepicker
  ...
  constructor(
    ...
    {
      this.isUpdate = true;
      this.dbFunction = 'updateAddonsTable';
...
saveAddon = () => {
  return new Promise<any>(async (resolve, reject) => {
    ...
    if (this.isUpdate) 
    {
      this.localInterfaceSrv.updateAddon(fields)
      .then(async res => {
        resolve(res);
        this.showSuccess('Addon Updated');
      })
      .catch(err => { 
        reject(err); 
    ...
    {
      this.localInterfaceSrv.addAddon(fields)
      .then(async res => {
        resolve(res);
        this.showSuccess('Addon Added');
      })
      .catch(err => { 
        reject(err); 
        this.toastr.error(err.description, "Error");
  ...
  public submitDB(content: object) {
    ...
    fd.append('addonId',this.mfData['id']);
    ...
    .then(
      data => {
              if (this.isUpdate) {this.showSuccess('Addon updated');} else {this.showSuccess('Addon added')}
      },
