...
export class DataTablesComponent {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe(async params => {
      ...
      await this.loadOwners(this.facilityId);
  });
    //this.getOwners();
  }
  @ViewChild('dangerModal', {static: false}) public dangerModal: ModalDirective;

  loadOwners = (facilityId) => {
    return new Promise<any>(async (resolve, reject) => {
      this.isLoading = true;
      this.localInterfaceSrv.getOwners(facilityId)
      .then(res => {
        this.dbData = res;
  ...
  deleteOwner = (userId) => {
    return new Promise<any>(async (resolve, reject) => {
      ...
      this.localInterfaceSrv.deleteOwner(userId)
      .then(async res => {
        resolve(res);
        this.toastr.success("Owner Removed", "Success");
  ...
  public async delete(content?) {
    ...
    return await this.httpService.post('deleteOwnersTable', fd, '')
    .then(
      data => {
        //window.location.reload()
      },
